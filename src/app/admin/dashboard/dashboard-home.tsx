        ...
        <CardContent className="pb-3">
            {isLoading ? (
                ...
                <>
                    ...
                    {description && <p className="text-[8px] text-muted-foreground mt-0.5 font-bold uppercase tracking-wider">{description}</p>}
                </>
            )}
        </CardContent>
    </Card>
...
export function DashboardHome({ profile }: { profile: { name?: string | null } | null }) {
    ...
    return (
        <div className="h-full flex flex-col space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700 min-h-0 overflow-hidden relative">
            <ComedyTip />
            
            <div className="flex flex-col gap-0 shrink-0">
                <h1 className="text-2xl font-black tracking-tighter">Welcome back, {profile?.name?.split(' ')[0] || 'Admin'}!</h1>
                <p className="text-muted-foreground text-xs font-medium">Site activity overview for today.</p>
            </div>

            <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 shrink-0">
                <StatCard 
                    title="Inbox" 
                    value={unreadCount} 
                    icon={MessagesSquare} 
                ...
                <StatCard title="Feedback" value={testimonials?.length ?? 0} icon={Star} isLoading={isLoading} description="Reviews" />
                <StatCard title="Services" value={services?.length ?? 0} icon={Sparkles} isLoading={isLoading} description="Capabilities" />
            </div>
            
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7 flex-1 min-h-0">
                <Card className="lg:col-span-4 border-border/50 shadow-sm flex flex-col overflow-hidden">
                    <CardHeader className="py-3 px-4 shrink-0">
                        <CardTitle className="text-sm font-bold">Recent Engagement</CardTitle>
                        <CardDescription className="text-[10px]">Weekly message frequency.</CardDescription>
                    </CardHeader>
                    <CardContent className="pl-2 flex-1 pb-2 min-h-0">
                        <RecentMessagesChart messages={messages} isLoading={isLoading} />
                    </CardContent>
                </Card>
                <Card className="lg:col-span-3 border-border/50 shadow-sm flex flex-col overflow-hidden">
                    <CardHeader className="py-3 px-4 shrink-0">
                        <CardTitle className="text-sm font-bold">Content Split</CardTitle>
                        <CardDescription className="text-[10px]">Creative portfolio distribution.</CardDescription>
                    </CardHeader>
                    <CardContent className="flex-1 flex items-center justify-center pb-2 min-h-0">
