...
export function ProjectCard({ project, index, onPreview, className }: ProjectCardProps) {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'group relative break-inside-avoid transition-all duration-700 ease-out h-full cursor-pointer',
    ...
    >
      <Card
        className="relative overflow-hidden transition-all duration-500 ease-in-out bg-[#141414] border-none rounded-[2.5rem] h-full flex flex-col shadow-2xl hover:shadow-primary/5"
      >
          ...
          <div className="relative w-full overflow-hidden aspect-[4/3]">
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20 flex items-center justify-center">
                <Button 
                    variant="outline" 
                    size="lg" 
                    className="rounded-full border-2 border-white/50 bg-black/30 backdrop-blur-md text-white hover:bg-white/20 hover:text-white transition-all duration-300 py-7 px-10 flex items-center gap-3 shadow-2xl scale-90 group-hover:scale-100 pointer-events-none"
                >
