...
export function Timeline() {
  ...
  const timelineCollectionRef = useMemoFirebase(() => {
    ...
    return query(collection(firestore, `admin_users/${adminUserId}/timeline_events`), orderBy('year', 'desc'));
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <div className="max-w-3xl mx-auto text-center mb-16">
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-primary mb-4">My Journey</p>
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-center">Career Milestones</h2>
          </div>
          ...
          <div className="relative max-w-4xl mx-auto">
            ...
            {timelineData && timelineData.map((item, index) => (
              <div key={item.id} className={cn(
                  "relative flex items-center mb-8 md:mb-6"
              )}>
                {/* Content Container */}
                <div className={cn(
                  ...
                  <span className="inline-block px-4 py-1 rounded-full bg-primary/5 text-primary text-[10px] font-black tracking-widest uppercase mb-3 md:mb-2 border border-primary/10">
                    {item.year}
                  </span>
            ...
            {!isLoading && timelineData?.length === 0 && (
                <div className="text-center text-muted-foreground p-12 border-2 border-dashed rounded-[2.5rem]">
